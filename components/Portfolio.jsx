...
const portfolioItems = [
  {
    ...
    thumbnail: "/placeholder.svg?height=180&width=120",
    type: "Graphic Design",
  },
  {
    id: 2,
...
export default function PortfolioSection() {
  ...
  const filteredItems =
    activeCategory === "All" ? portfolioItems : portfolioItems.filter((item) => item.type === activeCategory)
    ...
    <section className="py-16 md:py-24 px-4">
      <div className="container mx-auto">
        ...
        <div className="text-center mb-12">
          <h2 className="text-4xl md:text-5xl font-bold text-red-600 mb-2">Our Portfolio</h2>
          <p className="text-zinc-400">Here are some glimpse of our top work</p>
        </div>
          ...
          {categories.map((category) => (
            <button
              ...
              className={`px-6 py-2 rounded-full text-sm font-medium transition-colors ${
                activeCategory === category ? "bg-red-600 text-white" : "bg-zinc-900 text-white/70 hover:bg-zinc-800"
              }`}
            >
              {category}
            </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          {/* Main Display */}
          <div className="lg:col-span-12 grid grid-cols-1 lg:grid-cols-12 gap-6 bg-zinc-900/50 rounded-3xl p-6 mb-6">
            {/* Featured Image */}
            ...
            <div className="lg:col-span-7 flex flex-col justify-center">
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {selectedItem.tags.map((tag, index) => (
                  <span key={index} className="px-3 py-1 bg-zinc-800 rounded-full text-xs">
                    {tag}
                  </span>
                ))}
              </div>
          ...
          {filteredItems.map((item) => (
            <div
              ...
              className={`cursor-pointer rounded-xl overflow-hidden transition-all duration-300 ${
                selectedItem.id === item.id ? "ring-2 ring-red-600 scale-105" : "opacity-70 hover:opacity-100"
              }`}
            >
